...
void main() {
	...
	vec3 Lo = push.light_color.xyz * push.light_color.a; // Constant ambient light

	vec3 L = normalize(push.light_direction.xyz);
	vec3 H = normalize(V + L);

	vec3 radiance = push.light_color.xyz * push.light_direction.w;
	...
	float NdotL = max(dot(N, L), 0.0);
	Lo += (kD * C / PI + specular) * radiance * NdotL;

	vec3 light = Lo + C * emissive;
